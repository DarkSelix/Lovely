...
function deleteSelectedAiCharacter() {
    ...
    if (confirm(`Are you sure you want to delete the AI character: ${aiCharacters.find(c=>c.id === selectedId)?.name}?`)) {
...
function showUserPersonaForm(personaToEdit = null) {
    if (personaToEdit) {
        ...
        userPersonaDescriptionTextarea.value = personaToEdit.description;
    } else {
        editingUserPersonaId = null;
...
function showContextModal() {
    ...
    contextWindow.forEach(msg => {
        ...
        if (msg.role === 'user' && activeUserPersonaId) {
            const persona = userPersonas.find(p => p.id === activeUserPersonaId);
            if (persona && (msg.raw || getRawTextFromHtml(msg.content))) {
                 displayContent = `[User Persona: ${persona.description}]\n\n${msg.raw || getRawTextFromHtml(msg.content)}`;
            }
        }
...
async function sendMessage(userRawInput, userBubbleColor, addUserMessageToList = true) {
    ...
    const activePersona = userPersonas.find(p => p.id === activeUserPersonaId);
    if (activePersona && trimmedInput) {
        currentUserTurnContentForApi = `[User Persona: ${activePersona.description}]\n\n${trimmedInput}`;
    }
